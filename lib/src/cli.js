...
const COMMANDS = [
...

  {
    ...
    runFunction: () => {
      const backupDir = engine.getSetting('backup_dir');
      engine.openBackupDirectory()
        .then(() => {
          log([`Opened to this directory`, backupDir]);
        })
        .catch(() => {
          logError(`There was an error opening up the folder`);
          log([`You can find the directory here:`, backupDir], false);
        })
    }
  }
];
